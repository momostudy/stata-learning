...
capture program drop sfpan
program define sfpan
...
global hj_sfpan_dep "`varlist'"  /* for use in sf_pan_post.ado */
...
ml model d0 sf_pan (frontier: `varlist'=`frontier' $conss)  $mufun $gamfun (usigmas:`usigmas') (vsigmas:`vsigmas')  if `touse', `robust' $cl_ster technique("`thetech'")

global showM ml model d0 sf_pan (frontier: `varlist'=`frontier' $conss) $mufun $gamfun (usigmas:`usigmas') (vsigmas:`vsigmas') if `touse', `robust' $cl_ster technique("`thetech'")
global ML_user_hj sf_pan
