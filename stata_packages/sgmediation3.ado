...
`quietly' {
 ...
 `prefix' reghdfe `varlist' `iv' `cv' if `touse', absorb(stkcd#cohort year#cohort)
 est store m1
 *`prefix' xtreg `varlist' `iv' `cv' i.time if `touse', fe
 ...
 `prefix' reghdfe `mv' `iv' `cv' if `touse', absorb(stkcd#cohort year#cohort)
...
est store m2
 ...
 `prefix' reghdfe `varlist' `mv' `iv' `cv' if `touse', absorb(stkcd#cohort year#cohort)
 est store m3
 *`prefix' xtreg `varlist' `mv' `iv' `cv' i.time if `touse', fe

}
reg2docx m1 m2 m3 using "$output/机制检验.docx", replace ///
         scalars(N(%20.0fc) r2_a(%9.3f)) b(%9.4f) t(%7.3f) ///
         addfe("Firm×Cohort=Yes" "Year×Cohort=Yes") ///
